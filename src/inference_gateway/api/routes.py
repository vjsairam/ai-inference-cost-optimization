...
from inference_gateway.api.app import GatewayState
...
from inference_gateway.routing import RouteDecision
from inference_gateway.routing.policy import PolicyDenied, select_route
...
_access_log = logging.getLogger("inference_gateway.access")
...
def _error_response(
    ...
    headers = {"X-Gateway-Request-Id": request_id} if request_id else None
    return JSONResponse(
        status_code=status,
...
def register_routes(app: FastAPI) -> None:
    ...
    async def chat_completions(  # noqa: PLR0911 - one exit per normalized failure mode
        ...
        x_gateway_team: str | None = Header(default=None),
        x_gateway_workload: str | None = Header(default=None),
        x_gateway_data_class: str | None = Header(default=None),
        x_gateway_quality_tier: str | None = Header(default=None),
        x_gateway_request_id: str | None = Header(default=None),
    ) -> Response:
        state = _state(request)
        started = time.monotonic()
        request_id = (x_gateway_request_id or schemas.new_request_id())[:64]

        bearer = None
        if authorization and authorization.lower().startswith("bearer "):
            bearer = authorization[len("bearer ") :]
        try:
            identity = authenticate(state.auth, bearer, x_gateway_team)
        except AuthenticationFailed:
            return _error_response(401, "invalid or missing API key", "auth_failed", request_id)
        except TeamAssertionMismatch:
            return _error_response(
                403,
                "X-Gateway-Team does not match the authenticated identity",
                "team_mismatch",
                request_id,
            )
        team = identity.team

        workload = x_gateway_workload or "generic"
        if workload not in state.config.routing.workloads:
            return _error_response(422, "unknown workload", "invalid_workload", request_id)
        if x_gateway_data_class is None or x_gateway_quality_tier is None:
            return _error_response(
                422,
                "X-Gateway-Data-Class and X-Gateway-Quality-Tier are required",
                "missing_metadata",
                request_id,
            )
        try:
            data_class = DataClass(x_gateway_data_class)
            quality_tier = QualityTier(x_gateway_quality_tier)
        except ValueError:
            return _error_response(
